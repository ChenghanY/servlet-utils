...
/**
 * @Author james
 * @Description Servlet 打印 request 信息的工具类
 * @Date 2019/10/16
 */
public class RequestUtil_V1_1 {
    /**
     * 使用枚举类作为入参， 省略常量值校验
     */
    ...
    private INFO_TYPE infoType = INFO_TYPE.ALL;
    private String[] params;
    private Map<String, String[]> parameterMap;

    private RequestUtil_V1_1() {
    }
    ...
    private void setInfoType(INFO_TYPE infoType) {
    ...
    public static class Builder {
        RequestUtil_V1_1 servletRequestUtil = new RequestUtil_V1_1();
         ...
         * @see RequestUtil_V1_1
         */
        public RequestUtil_V1_1 build() {
            return servletRequestUtil;
        }

        public Builder buildInfoType(INFO_TYPE infoType){
    ...
    private static synchronized void printRequestInfo(HttpServletRequest request,
        ...
        if (INFO_TYPE.ALL == infoType) {
            appendCookiesInfo(request, parameterMap, sb);
            appendSessionInfo(request, sb);
        }
        if (INFO_TYPE.COOKIES == infoType) {
            appendCookiesInfo(request, parameterMap, sb);
        }
        if (INFO_TYPE.SESSION == infoType) {
            appendSessionInfo(request, sb);
        }
